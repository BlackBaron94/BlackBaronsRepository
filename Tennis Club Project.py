...
class Challenge:
    ...
    def requirements(self):
        ...
        if self.player1 - self.player2 > k5:
            ...
            return print(f"Ο παίκτης που προκαλείται είναι κάτω από τον παίκτη που προκαλεί. Η πρόκληση είναι άκυρη.")
        ...
        while answer not in ('ΝΑΊΝΑΙΟΧΙΌΧΙ'):
            answer = input(f"Νίκησε ο παίκτης στη θέση #{self.player1} που έκανε την πρόκληση; "
                           f"Παρακαλώ απαντήστε με 'Ναι' ή 'Όχι'. ").upper()
        if answer in ('ΝΑΊΝΑΙ'):
...
def delete_player(index):
    ...
    if empty_check(1):
        print("Η κατάταξη είναι άδεια!")
    elif empty_check(index): #Έλεγχος αν η θέση περιέχει άτομο
...
def rank_decay(index,today_string=today_string):
    ...
    if last_place == inactive_player[0][0]: #Έλεγχος για την περίπτωση που ο παίκτης είναι τελευταίος
        print("Ο παικτης {0} {1} είναι ήδη στην τελευταία θέση και δεν πέφτει περαιτέρω.".format(
